...
const Proxies = require("../frontend/proxies");
const {
  BACKEND,
  DELTAS,
  DELTAS_CACHE_MODE,
...
function createBottomDelta(frontend) {
  return [
    ...
    new CausalContext(frontend[REPLICA_ID]),
  ];
}
...
function init(options) {
  const frontend = {};
  const replicaId =
    options["REPLICA_ID"] || "r" + Math.floor(Math.random() * 1000);
  ...
  Object.defineProperty(frontend, DELTAS_CACHE_MODE, {
    value: deltasCacheMode,
  });
  Object.defineProperty(frontend, DELTAS, {
    value: [createBottomDelta(frontend)],
  });
...
function from(initialState, options) {
  return change(init(options), "Initialization", (doc) =>
...
function getChanges(frontend) {
  let deltaReducer = (accumulatedDelta, currentDelta) =>
    DotMap.join(accumulatedDelta, currentDelta);
